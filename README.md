...
`grand-5km-route-stickman-transparent.png`, a transparent 1448x1086 route PNG, as
the source image for `composePngBlob()`. The stickman is a placeholder event/brand
mark for this proof of concept: in a real race demo it could be swapped for a race
mascot, sponsor logo, medal, venue mark, or branded illustration. That gives the
event more value than a plain results table because every shared sticker becomes
lightweight promotion for the race or brand through a fun, personal medium.

Keep source images in `demo/assets/`; `demo-dist/` is generated by Vite and will be
overwritten on each production build.
